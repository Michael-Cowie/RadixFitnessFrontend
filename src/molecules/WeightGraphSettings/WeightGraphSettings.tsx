...
import { measurementSystemToUnit, weightFromKgToUserMeasurementSystem } from 'lib/weightTranslations';
...
const WeightGraphSettings: React.FC<Props> = ({ closeModalWindow }) => {
    ...
    const onSubmit = async(event: SyntheticEvent) => {
        ...
        const success = await updateGoalWeight(goalDate, weightInput);
        if (success) {
            setPartialState({
                ui: {
                    trendlineEnabled,
            ...
            <GroupContainer>
                <Group title='Display Options'>
                    <div className='flex flex-col'>
                        <CheckBoxWithLabel label='Goal Weight' name="goalWeightEnabled" defaultChecked={goalWeightEnabled} onChange={()=> setGoalWeightEnabled(!goalWeightEnabled)}/>
                        <SensitivityController sensitive={goalWeightEnabled }>
                            <div className='flex items-center space-x-1'>
                                <CheckBoxWithLabel label='Weight Prediction' name="enableWeightPrediction" checked={ enableWeightPrediction && goalWeightEnabled } onChange={() => setEnableWeightPrediction(!enableWeightPrediction)} />
                                <InformationHover information="Predictions are calculated from the visible weights on the graph."/>
                            </div>
                        </SensitivityController>
                        <CheckBoxWithLabel label='Trendline' name="trendlineEnabled" defaultChecked={defaultTrendlineEnabled} />
                    ...
                    <div className='mt-1 w-2/3 space-y-2'>
                        ...
                        <div className= { `${ !goalWeightEnabled ? styles.fadeImage : '' }` }>
                            <WeightTrackingSpinbutton
                                defaultValue={ weightFromKgToUserMeasurementSystem(goalWeightKg, measurementSystem) }
