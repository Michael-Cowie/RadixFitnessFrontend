...
import { convertKgTo, formatToTwoPrecision, measurementSystemToUnit, weightFromKgToUserMeasurementSystem } from 'lib/weightTranslations';
...
export function getNotesFromDate(
    date: string,
    dateToNotes: Record<string, string>
): string {
...
export function getWeightText(updating: boolean, formattedDate: string, userMeasurementSystem: MeasurementSystem, dateToWeightKg: Record<string, number> ): string {
    if (!updating) {
        return `Weight`;
    } 
    const weight = formatToTwoPrecision(getWeightFromDate(formattedDate, userMeasurementSystem, dateToWeightKg));
